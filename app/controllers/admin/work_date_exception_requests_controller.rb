class Admin::WorkDateExceptionRequestsController < ApplicationController
  ...
  def approve_request
    ...
    if exception_request.update(status: 1, approved_at: Time.current, approved_by_id: current_employee.id)
      ...
      redirect_to admin_work_date_exception_requests_path(@employee), notice: "承認に成功しました"
    else
      redirect_to admin_work_date_exception_requests_path(@employee), alert: "不明なエラー:employee_exception_request"
    end
  end
  ...
  def reject_request
    ...
    if exception_request.update(status: 2, approved_at: Time.current, approved_by_id: current_employee.id)
      redirect_to admin_work_date_exception_requests_path(@employee), notice: "却下に成功しました"
    else
      redirect_to admin_work_date_exception_requests_path(@employee), alert: "不明なエラー:employee_exception_request"
    end
  end
