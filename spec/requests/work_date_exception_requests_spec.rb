...
RSpec.describe "WorkDateExceptionRequests", type: :request do
  ...
  let(:user_1_exception) { FactoryBot.create(:work_date_exception_request, employee: user_1, request_type: 'paid_leave') }
  let(:user_2_exception) { FactoryBot.create(:work_date_exception_request, employee: user_2, request_type: 'paid_leave') }
  ...
  context "非ログイン時" do
    describe "GET user_1/employee_work_date_exception/new" do
      let(:request_action) { get new_employee_work_date_exception_request_path(user_1) }
      it_behaves_like "redirect_to_login_page"
    end

    describe "GET user_1/employee_work_date_exception/edit" do
      let(:request_action) { get edit_employee_work_date_exception_request_path(user_1, user_1_exception) }
      it_behaves_like "redirect_to_login_page"
    end
  end
  context "ユーザー1がログイン時" do
    ...
    describe "GET user_2/employee_work_date_exception/new" do
      let(:request_action) { get new_employee_work_date_exception_request_path(user_2) }
      let(:logged_in_employee) { user_1 }
    ...
    describe "GET user_2/employee_work_date_exception/edit" do
      let(:request_action) { get edit_employee_work_date_exception_request_path(user_2, user_2_exception) }
      let(:logged_in_employee) { user_1 }
      it_behaves_like "redirect_to_current_employee_attendance_index_page"
    end
  end
